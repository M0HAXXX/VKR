...
class IlluminationCalculatorApp:
    ...
    def create_task_page(self, task_num):
        ...
        # Структура для кнопки "Вернуться"
        button_frame = tk.Frame(main_frame, bg='#2c3e50')
        ...
        # Структура для содержимого (левая и правая панели)
...
Условия: l_ap ≤ l_sh, V >= 0"""
    ...
    def show_table(self, matrix, params, task_num):
        ...
        # Создание структураа с прокруткой
    ...
    def show_theory_page(self):
        ...
        # Создание структуры с прокруткой для текста
    ...
    def get_theory_sections(self):
        ...
        return [
            ("1. ДИСКРЕТИЗАЦИЯ ОСВЕЩЕННОСТИ",
             """Дискретизация освещенности - это процесс преобразования непрерывного распределения освещенности 
             в дискретные значения, соответствующие пикселям матрицы фотодиодов. 
             Каждый фотодиод имеет определенную апертуру и расположен с определенным шагом дискретизации."""),
            ...
            ("3. Задание 1 - Статический край",
             """Рассматривается случай перехода освещенности в точке a. 
             Освещенность для каждого пикселя определяется следующим образом:
